...
class _GamepageState extends State<Gamepage> {
      ...
      return Ink(
        ...
        decoration: BoxDecoration(
          gradient: LinearGradient(
              ...
              colors: [
                Color(0xffff00cc).withOpacity(.3),
                Color(0xff333399).withOpacity(.6)
              ]),
        ),
      );
  }
  ...
  void dialogueGameOver(int player) {
    if (_goLogic.highFiveAfter(player) > -1) definitiveDialogue(player);
    if (_boardState.isDraw()) drawDialogue();
  }
  ...
  Object definitiveDialogue(int playerMark) {
    String result = (playerMark == 0) ? 'Lost :(' : 'Won :D';
    gameOver = true;
    showDialog(
        context: context,
        builder: (_) => AlertDialog(
